...
const emptyStats = {
  ...
  totalExpenses: 0,
  newMembers: 0,
  expiring: [] as Array<{ id: string; name: string; phone: string; email: string; dueDate: string; amount: number }>,
};

export default function DashboardPage() {
  const { data: stats = emptyStats, isPending, isFetched } = useQuery({
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Bottom-right Copilot is free — try “Aaj kitne aaye?”</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-red-900/20 shadow-sm">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-emerald-900/20 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-6 mt-8">
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-8">
          ...
          <div className="space-y-4">
            <Link href="/dashboard/members" prefetch className="w-full text-left px-6 py-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-800 text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between group block">
              <span>Add New Member</span>
              <span className="text-xl font-light text-slate-400 group-hover:text-indigo-500">+</span>
            </Link>
            <Link href="/dashboard/fees" prefetch className="w-full text-left px-6 py-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-800 text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between group block">
              <span>Record Fee Payment</span>
              <span className="text-xl font-light text-slate-400 group-hover:text-indigo-500">+</span>
            </Link>
            <Link href="/dashboard/expenses" prefetch className="w-full text-left px-6 py-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/30 hover:bg-slate-100 dark:hover:bg-slate-800 text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between group block">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm p-8">
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center text-sm p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
